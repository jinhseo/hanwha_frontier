...
class FrontierDetector:
    def __init__(self):
        ...
        self.global_goal_x = rospy.get_param('/global_goal_x', -10.528198)
        self.global_goal_y = rospy.get_param('/global_goal_y', -60.882569)
    ...
    def grid_map_callback(self, msg):
        try:
            ...
            # GridMap의 pose가 (0,0)이므로 origin도 (0,0) 기준으로 설정
            # GridMap의 pose는 맵의 중심점이므로, 좌하단 모서리는 -width/2, -height/2
            self.origin_x = msg.info.pose.position.x - self.width/2
            self.origin_y = msg.info.pose.position.y - self.height/2

            # 디버깅: 원본 GridMap 정보 출력
    ...
    def compute_cost_map(self, incl, coll, steep):
        """Frontier 탐색에 특화된 traversability map 생성"""
        ...
        for i in range(incl.shape[0]):
            for j in range(incl.shape[1]):
                ...
                if np.isnan(incl_val) or np.isnan(coll_val) or np.isnan(steep_val):
                    ...
                    # Frontier 탐색용 단순한 traversability 계산
                    # 1. 충돌 위험이 높으면 주행 불가
                    if coll_val > 0.8:  # 충돌 위험 임계값
                        traversability_map[i, j] = 1.0  # 주행 불가
                    # 2. 경사가 너무 심하면 주행 불가
                    elif steep_val > 0.7 or incl_val > 0.7:  # 경사 임계값
                        traversability_map[i, j] = 1.0  # 주행 불가
                    # 3. 약간의 위험이 있지만 주행 가능
                    elif coll_val > 0.3 or steep_val > 0.3 or incl_val > 0.3:
